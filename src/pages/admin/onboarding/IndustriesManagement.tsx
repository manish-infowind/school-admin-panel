...
import { Industry, CreateReferenceDataRequest, UpdateReferenceDataRequest } from "@/api/types";
...
export default function IndustriesManagement() {
  ...
  const { data: industries = [], isLoading } = useIndustries({ includeInactive });
  ...
  const openCreateModal = () => {
    setFormData({
      ...
      label: "",
      description: "",
      sortOrder: industries.length > 0 ? Math.max(...industries.map(i => i.sortOrder)) + 1 : 0,
      active: true,
    });
  ...
  const handleCreate = () => {
    ...
    createIndustry.mutate(formData, {
      onSuccess: () => {
        closeCreateModal();
      },
    });
  ...
  const handleUpdate = () => {
    if (!selectedIndustry || !formData.code.trim() || !formData.label.trim()) {
      toast({
        title: "Error",
        description: "Code and label are required",
        variant: "destructive",
    ...
    updateIndustry.mutate(
      { id: selectedIndustry.id, data: formData },
      {
        onSuccess: () => {
          closeEditModal();
        },
  ...
  const handleDelete = () => {
    ...
    deleteIndustry.mutate(industryToDelete.id, {
      onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        page="industries"
        heading="Industries Management"
        subHeading="Manage industries for onboarding"
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            <PageLoader pagename="industries" />
          ) : (
                  ...
                  {filteredIndustries.length === 0 ? (
                    ...
                    filteredIndustries.map((industry) => (
                      <TableRow key={industry.id}>
                        ...
                        <TableCell>{industry.sortOrder}</TableCell>
                        <TableCell>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Lowercase with underscores only (e.g., fintech, edtech)
              </p>
            </div>
            <div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Are you sure you want to delete the industry <strong>{industryToDelete?.label}</strong>?
            </p>
            <div className="flex justify-end gap-2">
